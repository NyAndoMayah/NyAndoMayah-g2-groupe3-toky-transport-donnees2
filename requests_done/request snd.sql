...
SELECT sum(essence+maintenance) AS Depense, count(matricule) AS nb_voyage_fait FROM vehicule INNER JOIN recevoir ON vehicule.id_vehicule = recevoir.id_vehicule
INNER JOIN depense ON depense.id_depense = recevoir.id_depense
INNER JOIN voyage ON vehicule.id_vehicule = voyage.id_vehicule
